...
file 'README.md' => 'package.json' do |t|
  ...
  content = content.gsub(%r{(mmistakes/minimal-mistakes@)[\d.]+}, "\\1#{package_json['version']}")
  File.write(t.name, content)
end

file 'docs/_pages/home.md' => 'package.json' do |t|
  content = File.read(t.name)
  content = content.gsub(%r{(\breleases/tag/|Latest release v)[\d.]+}, "\\1#{package_json['version']}")
  File.write(t.name, content)
end
